...
from src.diffusion.nn import mean_flat, sum_flat
from src.diffusion.losses import normal_kl, discretized_gaussian_log_likelihood

from smplx import SMPLXLayer
from src.utils.rotation_conversion import sixd2rot_torch,rot2sixd_torch
...
class GaussianDiffusion:
    ...
    def training_losses(self, model, x_start, t, model_kwargs=None, noise=None, dataset=None):
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            assert model_output.shape == target.shape == x_start.shape  # [bs, nframes, d]
            ...
            if self.lambda_vel>0:
                taget_vel=target_pos[:,:,:,1:]-target_pos[:,:,:,:-1]
            ...
            if self.lambda_geo>0:
                def get_transformed_verts(states,verts):
                    """
                    states: [bs, 1,9,nframes]
                    verts: [bs, 1024, 3]
                    """
                    states=states.squeeze(1).permute(0,2,1) # [bs, 9, nframes]

                    # 1. get rotmat and transl
                    rot6d,transl=states[:,:,:6],states[:,:,6:] # transl [bs, nframes, 3]
                    rotmat=sixd2rot_torch(rot6d) # [bs, nframes, 3, 3]
                    # 2. transform verts
                    verts=verts.unsqueeze(1).repeat((1,rot6d.shape[1],1,1)) # [bs, nframes, 1024, 3]
                    rotmat=rotmat.unsqueeze(2).repeat((1,1,1024,1,1)) # [bs, nframes, 1024, 3, 3]
                    transl=transl.unsqueeze(2).repeat((1,1,1024,1)) # [bs, nframes, 1024, 3]
                    transformed_verts=torch.matmul(rotmat,verts.unsqueeze(-1)).squeeze(-1)+transl # [bs, nframes, 1024, 3]
                ...
                if self.train_args.network.split('_')[-1]=='2o':
                    ...
                    target_obj2_state=target[:,:,471+9:471+9+9,:]
                    target_obj3_state=target[:,:,471+9+9:471+9+9+9,:] 
                    model_output_obj1_state=model_output[:,:,471:471+9,:]
                    model_output_obj2_state=model_output[:,:,471+9:471+9+9,:]
                    model_output_obj3_state=model_output[:,:,471+9+9:471+9+9+9,:]
                    obj1_sampled_verts=model_kwargs['y']['obj1_sampled_verts'] # [bs,1024,3]
                    obj2_sampled_verts=model_kwargs['y']['obj2_sampled_verts']
                    obj3_sampled_verts=model_kwargs['y']['obj3_sampled_verts']

                    target_obj1_verts=get_transformed_verts(target_obj1_state,obj1_sampled_verts) # [bs, 1024, 3, nframes]
                    target_obj2_verts=get_transformed_verts(target_obj2_state,obj2_sampled_verts)
                    target_obj3_verts=get_transformed_verts(target_obj3_state,obj3_sampled_verts)
                    model_output_obj1_verts=get_transformed_verts(model_output_obj1_state,obj1_sampled_verts)
                    model_output_obj2_verts=get_transformed_verts(model_output_obj2_state,obj2_sampled_verts)
                    model_output_obj3_verts=get_transformed_verts(model_output_obj3_state,obj3_sampled_verts)
                    terms["obj1_geo_mse"]=self.masked_l2(target_obj1_verts, model_output_obj1_verts, mask)
                    terms["obj2_geo_mse"]=self.masked_l2(target_obj2_verts, model_output_obj2_verts, mask)
                    terms["obj3_geo_mse"]=self.masked_l2(target_obj3_verts, model_output_obj3_verts, mask)
                else:
                    raise NotImplementedError(self.train_args.network)
            if self.lambda_sp>0:
                if self.train_args.network.split('_')[-1]=='2o':
                    target_dirs=target_obj1_verts-target_obj2_verts # [bs, 1024, 3, nframes]
                    model_output_dirs=model_output_obj1_verts-model_output_obj2_verts 
                    target_distance=torch.norm(target_dirs,dim=2,keepdim=True) # [bs, 1024,1, nframes]
                    model_output_distance=torch.norm(model_output_dirs,dim=2,keepdim=True) # [bs, 1024,1, nframes]
                    terms["obj1_obj2_sp"]=self.masked_l2(target_distance, model_output_distance, mask)
                elif self.train_args.network.split('_')[-1]=='3o':
                    def get_sp(target_1_verts,target_2_verts,model_out_1_verts,model_out_2_verts,mask):
                        target_dirs=target_1_verts-target_2_verts
                        target_distance=torch.norm(target_dirs,dim=2,keepdim=True)
                        model_output_dirs=model_out_1_verts-model_out_2_verts
                        model_output_distance=torch.norm(model_output_dirs,dim=2,keepdim=True)
                        return self.masked_l2(target_distance, model_output_distance, mask)
                    terms["obj1_obj2_sp"]=get_sp(target_obj1_verts,target_obj2_verts,model_output_obj1_verts,model_output_obj2_verts,mask)
                    terms["obj2_obj3_sp"]=get_sp(target_obj2_verts,target_obj3_verts,model_output_obj2_verts,model_output_obj3_verts,mask)
                    terms["obj3_obj1_sp"]=get_sp(target_obj3_verts,target_obj1_verts,model_output_obj3_verts,model_output_obj1_verts,mask)
                else:
                    raise NotImplementedError(self.train_args.network)
            terms["loss"]=self.lambda_recon*terms.get("recon_mse",0)+\
                        self.lambda_pos*terms.get("pos_mse",0)+\
                        self.lambda_vel*terms.get("vel_mse",0)+\
                        self.lambda_geo*terms.get("obj1_geo_mse",0)+\
                        self.lambda_geo*terms.get("obj2_geo_mse",0)+\
                        self.lambda_geo*terms.get("obj3_geo_mse",0)+\
                        self.lambda_sp*terms.get("obj1_obj2_sp",0)+\
                        self.lambda_sp*terms.get("obj2_obj3_sp",0)+\
                        self.lambda_sp*terms.get("obj3_obj1_sp",0)
        else:
